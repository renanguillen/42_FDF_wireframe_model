...
/*   Updated: 2022/09/16 20:35:59 by ridalgo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_points(t_img *img, t_data *data, int i, int j)
{
	if (j != data->cols - 1)
	{
		img->x0 = data->dot[i][j].x + data->travelx;
		img->y0 = data->dot[i][j].y + data->travely;
		img->color0 = data->dot[i][j].color;
		img->x1 = data->dot[i][j + 1].x + data->travelx;
		img->y1 = data->dot[i][j + 1].y + data->travely;
		img->color1 = data->dot[i][j + 1].color;
		set_line_angle(img);
	}
	if (i != data->rows -1)
	{
		...
		img->y0 = data->dot[i][j].y + data->travely;
		img->color0 = data->dot[i][j].color;
		img->x1 = data->dot[i + 1][j].x + data->travelx;
		img->y1 = data->dot[i + 1][j].y + data->travely;
		img->color1 = data->dot[i + 1][j].color;
		set_line_angle(img);
	}
}
